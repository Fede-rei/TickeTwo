...
CREATE EVENT IF NOT EXISTS pulizia_eventi_scaduti
    ON SCHEDULE EVERY 1 DAY
    DO
    BEGIN
        -- Elimina i record dalla tabella Acquisti legati agli eventi scaduti
        DELETE FROM Acquisti WHERE id_ticket IN (SELECT id_Biglietto FROM Biglietto WHERE id_event IN (SELECT id_evento FROM Evento WHERE data < NOW()));

        -- Elimina i record dalla tabella Carrello legati agli eventi scaduti
        DELETE FROM Carrello WHERE id_biglietto IN (SELECT id_Biglietto FROM Biglietto WHERE id_event IN (SELECT id_evento FROM Evento WHERE data < NOW()));

        -- Elimina i record dalla tabella Biglietto legati agli eventi scaduti
        DELETE FROM Biglietto WHERE id_event IN (SELECT id_evento FROM Evento WHERE data < NOW());

        -- Elimina i record dalla tabella Evento che sono scaduti
        DELETE FROM Evento WHERE data < NOW();
    END;
